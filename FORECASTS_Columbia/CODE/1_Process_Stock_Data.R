# SCRIPT TO DOWNLOAD AND REORGANIZE THE COLUMBIA DATA
...
if(!dir.exists("FORECASTS_Columbia/OUTPUT")){dir.create("FORECASTS_Columbia/OUTPUT")}
if(!dir.exists("FORECASTS_Columbia/DATA/1_Downloads")){dir.create("FORECASTS_Columbia/DATA/1_Downloads")}
if(!dir.exists("FORECASTS_Columbia/DATA/2_Lookup_Files")){dir.create("FORECASTS_Columbia/DATA/2_Lookup_Files")}
if(!dir.exists("FORECASTS_Columbia/DATA/3_ProcessedData")){dir.create("FORECASTS_Columbia/DATA/FORECASTS_Columbia/DATA/3_ProcessedData")}



columbia.path <- getURL("https://raw.githubusercontent.com/SOLV-Code/SalmonPrize_DataManagement/refs/heads/main/DATA/2026_Sockeye_International/Columbia/DataPackage_Columbia_2026/FullDataSet_LongFormat.csv")
columbia.src <- read.csv(text = columbia.path)
names(columbia.src )

write_csv(columbia.src , "FORECASTS_Columbia/DATA/1_Downloads/FullDataSet_LongFormat.csv")


stk.lookup <-columbia.src  %>% select(System, River) %>% unique()
write_csv(stk.lookup,
          "FORECASTS_Columbia/DATA/2_Lookup_Files/GENERATED_Stock_Lookup.csv")


age.class.lookup <- read_csv("FORECASTS_Columbia/DATA/2_Lookup_Files/MANUAL_AgeClass_Lookup.csv")


# get range of % age class by brood year

age.comp.details <- columbia.src %>% group_by(System,River,Label) %>%
      summarize(
...
write_csv(age.comp.details,"FORECASTS_Columbia/DATA/3_ProcessedData/GENERATED_AgeComp_Details.csv")
...
age.lookup.filename <- "FORECASTS_Columbia/DATA/3_ProcessedData/GENERATED_AgeComp_MainAgeClasses.csv"
...
pooled.data.long <- columbia.src %>% group_by(System,River,ReturnYear,Age) %>%
...
write_csv(pooled.data.long,"FORECASTS_Columbia/DATA/3_ProcessedData/GENERATED_PooledByAge_LongForm.csv")
...
for(stk.do in stk.list.all){
  ...
  for(fc.yr.do in 2021:2026){
    ...
    if(!dir.exists("FORECASTS_Columbia/DATA/3_ProcessedData/ForecastR_InputFiles")){
      dir.create("FORECASTS_Columbia/DATA/3_ProcessedData/ForecastR_InputFiles")}

    write_csv(pooled.data.sub,
              paste0("FORECASTS_Columbia/DATA/3_ProcessedData/ForecastR_InputFiles/ForecastR_Input_",
